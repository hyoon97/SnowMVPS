...
class MVSDataset(Dataset):
            ...
            for _ in range(self.nlights):
                with open('diligent_mv_pairs.txt') as f:
                    ...
                    for _ in range(num_viewpoint):
                        ...
                        lights = sorted(random.sample(range(1, 97), 1))
                        # lights = sorted(random.sample(range(4, 5), self.nlights))
                        # lights = sorted(random.sample(range(1, 6), self.nlights))
    ...
    def __getitem__(self, item):
        ...
        for i, vid in enumerate(view_ids):
            ...
            # rotation_matrices.append(R) # for single light condition
            ...
            # view_imgs = view_imgs[0] # when using only single light condition
            ...
            for ind, light in enumerate(lights):  # for each light
                ...
                imgs.append(img)
                light_dirs.append(l_dirs[light - 1])
                if i==0:
                    break
            ...
            # proj_matrices.append(proj_mat) # if use only single light condition
            for ind, light in enumerate(lights):  # for each light
        ...
        # stage4_pjmats[:, 1, :2, :] = proj_matrices[:, 1, :2, :] * 4.0
