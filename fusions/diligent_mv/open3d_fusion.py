...
def load_data(root_path, depth_path, scene_name, thresh):
    ...
    for view in range(20):
        if view%sparsity == 0:
            img_filename = "{}/{}/images/{:02d}.jpg".format(depth_path, scene_name, view)
            cam_filename = "{}/{}/cams/{:02d}_cam.txt".format(depth_path, scene_name, view)
            depth_filename = "{}/{}/depth_est/{:02d}.pfm".format(depth_path, scene_name, view)
            confidence_filename = "{}/{}/confidence/{:02d}.pfm".format(depth_path, scene_name, view)
...
def open3d_filter():
    with torch.no_grad():
        ...
        for i, scene in enumerate(all_scenes):
            ...
            write_ply('{}/mvsnet{}.ply'.format(args.ply_path, ply_id), np.concatenate(points, axis=0))
            del points, depths, rgbs, projs

            gc.collect()

            # print('Save {}/mvsnet{:03d}.ply successful.'.format(args.ply_path, ply_id))
            print('Save {}/mvsnet{}.ply successful.'.format(args.ply_path, ply_id))
