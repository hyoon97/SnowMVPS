...
LOG_DIR="./checkpoints/_mv/"$THISNAME 
DTU_OUT_DIR="./outputs/"$THISNAME

if [ $FUSION_METHOD = "pcd" ] ; then
python3 fusions/dtu/pcd.py ${@} \
    --testpath=$DTU_TEST_ROOT --testlist="datasets/lists/diligent_mv/test.txt" \
...
CUDA_VISIBLE_DEVICES=0 python fusions/diligent_mv/open3d_fusion.py --device="cuda" \
    --root_path=$DTU_TEST_ROOT \
    --depth_path=$DTU_OUT_DIR \
    --data_list="lists/diligent_mv/test.txt" \
    \
