...
parser.add_argument('--testpath', default='/ssd3/hsy/Dataset/MVPS_dataset_2022b_vulcan/', help='test datapath')
...
parser.add_argument('--loadckpt', default='/ssd4/seongmin/workspace/SnowMVPS/checkpoints/mvsp-attention1/2.ckpt', help='load a specific checkpoint')
...
# parser.add_argument('--dataset', default='general_eval4_synthetic_ps_stage4', help='select dataset')
# parser.add_argument('--testpath', default='/ssd3/hsy/Dataset/MVPS_dataset_2022b_vulcan/', help='train datapath')
# parser.add_argument('--testlist', default='/ssd3/hsy/Dataset/MVPS_dataset_2022b_vulcan/synthetic_pairs.txt', help='test list')
...
def save_scene_depth(testlist):
    ...
    if args.dataset.startswith('general_eval4_diligent'):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            outputs['ps_normal'] = ps_normal_est
            ...
            for filename, cam, img, depth_est, mvs_normal_est, ps_normal_est, photometric_confidence, weight, vals in zip(filenames, cams, imgs,  \
                                                            outputs["depth"], outputs["normal_plane"], outputs['ps_normal'], outputs["photometric_confidence"],outputs["weight"],outputs["val"]):
                #imgs = img.numpy()
                imgs = img.copy()
                # img = img[0]  #ref view
                ...
                normal_ps_filename = os.path.join(args.outdir, filename.format('normal_ps', '.pfm'))
                normal_mvs_filename = os.path.join(args.outdir, filename.format('normal_mvs', '.pfm'))
                ...
                os.makedirs(normal_ps_filename.rsplit('/', 1)[0], exist_ok=True)
                os.makedirs(normal_mvs_filename.rsplit('/', 1)[0], exist_ok=True)
                ...
                # save ps normal maps
                ps_normal_est = -ps_normal_est[0].transpose((1,2,0))
                save_pfm(normal_ps_filename, ps_normal_est)

                ps_normal_est = cv2.cvtColor(ps_normal_est, cv2.COLOR_RGB2BGR)
                cv2.imwrite(normal_ps_filename[:-3]+"jpg", (ps_normal_est + 1) * 127.5 * mask[..., None])

                # save mvs normal maps
                mvs_normal_est = -mvs_normal_est.transpose((1,2,0))
                save_pfm(normal_mvs_filename, mvs_normal_est)

                mvs_normal_est = cv2.cvtColor(mvs_normal_est, cv2.COLOR_RGB2BGR)
                cv2.imwrite(normal_mvs_filename[:-3]+"jpg", (mvs_normal_est + 1) * 127.5 * mask[..., None])
