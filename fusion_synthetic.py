...
def load_cam_params(params, scene):
    xmls_path = os.path.join(args.testpath, 'xmls', 'test')

    intrinsics = read_intrinsics(os.path.join(xmls_path, scene,
                                                    'intrinsics.txt'))
    extrinsics = read_extrinsics(os.path.join(xmls_path, scene,
                                                    'cam_to_worlds.npy'))
    
    params['intrinsics'] = intrinsics
    params['c2w'] = extrinsics
...
def filter_depth(scan_folder, out_folder, scene, save_folder=None):
    # the pair file
    pair_file = os.path.join(args.testpath, "synthetic_pairs.txt")
    ...
    ref_params = {}
    ref_params = load_cam_params(ref_params, scene)
    for ref_view, src_views in pair_data:
        
        # load the camera parameters
        ref_intrinsics = ref_params['intrinsics'].copy()
        ref_intrinsics[0][2] -= 50  # cx
        c2w = ref_params['c2w'][ref_view].copy()  # (4, 4) # c2w
        # flip because of mitsuba coordinates
        # mitsuba: x -> left, y -> up, z -> towards
        c2w = c2w * np.array([-1, -1, 1, 1]).reshape((1, 4)).astype(np.float32)
        ref_extrinsics = np.linalg.inv(c2w)
        ref_R = ref_extrinsics[:3,:3]
        

        # load the reference image. Use light 3
        ref_img = read_img(os.path.join(scan_folder, f'image_{ref_view:03d}_l03.exr'))
        ...
        geo_mask_sum = 0
        src_params = {}
        src_params = load_cam_params(src_params, scene)
        for src_view in src_views:
            
            # load the camera parameters
            src_intrinsics = src_params['intrinsics'].copy()
            src_intrinsics[0][2] -= 50  # cx
            c2w = src_params['c2w'][src_view].copy()  # (4, 4) # c2w
            # flip because of mitsuba coordinates
            # mitsuba: x -> left, y -> up, z -> towards
            c2w = c2w * np.array([-1, -1, 1, 1]).reshape((1, 4)).astype(np.float32)
            src_extrinsics = np.linalg.inv(c2w)
        ...
        # object mask
        object_mask = read_mask(os.path.join(scan_folder, f'mask_{ref_view:03d}.exr'))
...
def step2(save_folder):
    for scene in ['scene_000000', 'scene_000001', 'scene_000002', 'scene_000003']:
        scan_folder = os.path.join(args.testpath, 'dataset', 'test', scene)
