...
class MVS4net(nn.Module):
    def __init__(self, arch_mode="fpn", reg_net='reg2d', num_stage=4, fpn_base_channel=8, 
        ...
        self.confinet = ConfidenceNet(input_depths=self.stage_splits[2], input_channel = self.group_cor_dim[2])
    ...
    def forward(self, imgs, view_feats, rotation, proj_matrices, depth_values, pred_normal, filename=None):
        ...
        for stage_idx in range(self.num_stage):
            if not self.asff:
                ...
                features_stage = view_feats["stage{}".format(stage_idx+1)]
            else:
                # features_stage = [self.asff[stage_idx](feat['stage1'],feat['stage2'],feat['stage3'],feat['stage4']) for feat in features]
                features_stage = [self.asff[stage_idx](feat['stage1'],feat['stage2'],feat['stage3'],feat['stage4']) for feat in view_feats]

            proj_matrices_stage = proj_matrices["stage{}".format(stage_idx + 1)]
            # print(len(view_feats['stage1']))
            # print(len(features_stage))
            # print(features_stage[0].shape)
            # raise
...
def MVS4net_loss(depth_values, inputs, ps_normal_est, depth_gt_ms, mask_ms, normal_gt, **kwargs):
    ...
    normal_loss = torch.tensor(0.0, dtype=torch.float32, device=mask_ms["stage1"].device, requires_grad=False)
    stage_ot_loss = []
    stage_l1_loss = []
    range_err_ratio = []
    stage_normal_loss = []
    ...
    for stage_idx, (stage_inputs, stage_key) in enumerate([(inputs[k], k) for k in inputs.keys() if "stage" in k]):
        ...
        omask = mask_stage > 0.5
        mask = omask * re_mask.unsqueeze(1)

        normal_mask = mask.unsqueeze(2).repeat(1,1,3,1,1) # (b, v, 3, h, w)
        ...
        pw_loss = pixel_wise_loss(prob_volume, depth_gt, mask[:, 0], hypo_depth, valid_vol_mask, stage_idx)  # reference view
